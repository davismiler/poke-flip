<template>
  ...
  <div class="flex flex-col min-h-screen">
    <PokeHeader />
    <div class="contain mx-auto flex flex-1 items-center p-4 text-foreground tracking-widest">
    ...
    <div class="card-not-pokemons justify-center w-full">
      <div class="flex flex-col justify-center items-center max-w-md">
        <img class="rounded-lg" src="/images/sad-pikachu.gif" alt="pikachu" />
        <span class="text-foreground mt-6 block text-center text-base"> Currently not available for mobile devices </span>
      </div>
    </div>
    <div class="w-full pokemons-view flex-col">
      ...
      <div class="pokemons-view relative z-10 justify-center bg-card min-h-[35rem] border-[3px] border-border min-w-[1000px]">
        <PokeSidebar>
          <template #header>
            <PokeLogo />
          </template>
...
<script setup>
  ...
  const difficultyOptions = ref(['normal', 'hard', 'extreme'])
  ...
  const comparePokemons = (pokemon1, pokemon2) => {
    ...
    if (pokemon1.id === pokemon2.id) {
      ...
      // check if there are still pokemons to finish the game
    ...
    if (pokemon1.id !== pokemon2.id) {
      ...
      setTimeout(() => {
        ...
        evaluatePokemons.value = []
        isComparing.value = false
        if (difficultySelected.value === 'extreme') {
          shuffleArrayPokemons(pokemons.value)
        }
      }, 1000)
    }
